...
#add noise on training step
println("Training on noisy data and testing on noiseless data...")
train_in += 0.045*randn(size(train_in, 1), size(train_in, 2))
#train_out = add_noise(train_out, 0.045)
...
sig = var(data)
println("SVESM nmrse for noisy training and noiseless testing: ", nrmse(test_out, output, sig))
svesm_nn = plot(output, label="SVESM")
svesm_nn = plot!(test_out, label="actual")
savefig(svesm_nn, "svesm_noisytrain_comparison")
...
println("SVM Poly kernel nmrse for noisy training and noiseless testing: ", nrmse(test_out, t1, sig))
println("SVM RadialBasis kernel nmrse for noisy training and noiseless testing: ", nrmse(test_out, t2, sig))
svm_nn = plot(t1, label="SVM Polynomial kernel")
svm_nn = plot!(t2, label="SVM RadialBasis kernel")
svm_nn = plot!(test_out, label="actual")
savefig(svm_nn, "svm_noisytrain_comparison")
