...
function ESN(
    ...
    input_matrix, reservoir_matrix, bias_vector, res_size = obtain_layers(
        in_size, input_layer, reservoir, bias; matrix_type=matrix_type
    )

    inner_res_driver = reservoir_driver_params(reservoir_driver, res_size, in_size)
    states = create_states(
        inner_res_driver, train_data, washout, reservoir_matrix, input_matrix, bias_vector
    )
...
function obtain_layers(
    ...
    input_matrix = create_layer(
        input_layer, input_res_size, in_size, matrix_type=matrix_type
    )
...
function obtain_layers(
    ...
    if input_layer isa Array
        input_matrix = [create_layer(
            input_layer[j], input_res_sizes[j], in_sizes[j], matrix_type=matrix_type
        ) for j in 1:esn_depth]
    else
        _input_layer = fill(input_layer, esn_depth)
        input_matrix = [create_layer(
            _input_layer[k], input_res_sizes[k], in_sizes[k], matrix_type=matrix_type
        ) for k in 1:esn_depth]
    end

    res_sizes = [get_ressize(input_matrix[j]) for j in 1:esn_depth]
    reservoir_matrix = [create_reservoir(
        reservoir[k], res_sizes[k], matrix_type=matrix_type
    ) for k in 1:esn_depth]

    if bias isa Array
        bias_vector = [create_layer(
            bias[j], res_sizes[j], 1, matrix_type=matrix_type
        ) for j in 1:esn_depth]
    else
        _bias = fill(bias, esn_depth)
        bias_vector = [create_layer(
            _bias[k], res_sizes[k], 1, matrix_type=matrix_type
        ) for k in 1:esn_depth]
    end
...
function (esn::ESN)(
    ...
    if variation isa Hybrid
        ...
        return obtain_esn_prediction(
            esn, prediction, last_state, output_layer, model_pred_data; initial_conditions=initial_conditions
        )
    else 
        return obtain_esn_prediction(
            esn, prediction, last_state, output_layer; initial_conditions=initial_conditions
        )
    end
end
