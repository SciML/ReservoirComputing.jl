function obtain_esn_prediction(
    esn,
    ...
    for i in 1:prediction_len
        x, x_new = next_state_prediction!(
            esn, x, x_new, out, out_pad, i, tmp_array, args...
        )
...
function obtain_esn_prediction(
    ...
    for i in 1:prediction_len
        x, x_new = next_state_prediction!(
            esn, x, x_new, prediction.prediction_data[:,i], out_pad, i, tmp_array, args...
        )
...
function next_state_prediction!(esn::ESN, x, x_new, out, out_pad, i, tmp_array, args...)
    return _variation_prediction!(
        esn.variation, esn, x, x_new, out, out_pad, i, tmp_array, args...
    )
end

#dispatch the prediction on the esn variation
function _variation_prediction!(
    variation,
    esn,
    x,
    x_new,
    ...
    x = next_state!(
        x, esn.reservoir_driver, xv, out_pad,
...
function _variation_prediction!(
    ...
    x = next_state!(
        x, esn.reservoir_driver, x[1:esn.res_size], out_pad,
        esn.reservoir_matrix, esn.input_matrix, esn.bias_vector, tmp_array
    )
