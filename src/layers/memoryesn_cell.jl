@doc raw"""
    MemoryESNCell(in_dims => out_dims, [activation];
...
\begin{aligned}
    \mathbf{x}(t) &= (1-\alpha)\, \mathbf{x}(t-1)
        ...
        + \mathbf{W}_r\, \mathbf{x}(t-1) + \mathbf{W}_m\, \mathbf{m}(t-1)
        + \mathbf{b} \right)
\end{aligned}
```
## Arguments
...
function initialparameters(rng::AbstractRNG, esn::MemoryESNCell)
    ps = (
...
function (esn::MemoryESNCell)(inp::AbstractArray, ps, st::NamedTuple)
...
function Base.show(io::IO, esn::MemoryESNCell)
    print(io, "MemoryESNCell($(esn.in_dims) => $(esn.out_dims)")
