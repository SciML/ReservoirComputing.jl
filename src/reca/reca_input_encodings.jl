...
Random mapping of the input data directly in the reservoir. The ```expansion_size``` determines the dimension of the single reservoir, and 
```permutations``` determines the number of total reservoirs that will be connected, each with a different mapping. The detail of this 
